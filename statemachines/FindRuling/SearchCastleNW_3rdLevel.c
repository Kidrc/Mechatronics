...
#define SEARCH_CASTLE_NW_ENUMFORM(STATE) SEARCH_CASTLE_NW_##STATE,

#define LIST_OF_SEARCH_CASTLE_NW_STATES(STATE)\
    STATE(SEARCH)\
    STATE(DETECTING)\
    STATE(CW90)\
    STATE(RETURN_CENTER)\
...
typedef enum
{
    INIT_SEARCH_CASTLE_NW_STATE=0, LIST_OF_SEARCH_CASTLE_NW_STATES(SEARCH_CASTLE_NW_ENUMFORM)
}SubSearchNW_t;

static SubSearchNW_t currentState;

ES_Event SEARCH_CASTLE_NW_RunSubHSM(ES_Event ThisEvent);

uint8_t SEARCH_CASTLE_NW_InitSubHSM(void)
{
    ES_Event returnEvent;

    currentState = INIT_SEARCH_CASTLE_NW_STATE;
    returnEvent = SEARCH_CASTLE_NW_RunSubHSM(INIT_EVENT);
    dbugprintf("SEARCH_CASTLE_NW_INITFUNCTION, currentState = %d\n", currentState);
...
ES_Event SEARCH_CASTLE_NW_RunSubHSM(ES_Event ThisEvent)
{
    ...
    switch(currentState)
    {
        case INIT_SEARCH_CASTLE_NW_STATE:
            if(ThisEvent.EventType == ES_INIT)
            {
                currentState = SEARCH_CASTLE_NW_CW90;
                ThisEvent.EventType = ES_NO_EVENT;
                dbugprintf("INIT_SEARCH_CASTLE_NW = %d\n", currentState);
                //makeTransition = TRUE;
            }
            break;

        case SEARCH_CASTLE_NW_CW90:
            if (ThisEvent.EventType != ES_NO_EVENT)
            {
                switch(ThisEvent.EventType)
                {
                 /*   case ES_ENTRY:
                        dbugprintf("SEARCH_CASTLE_NW_CCW90\n");
                    ...
                    case BUMPED:
                        if (BUMPER(BUMPER_LEFT))
                        {
                            ...
                            nextState = SEARCH_CASTLE_NW_DEADBOT_TURNING;
                            makeTransition = TRUE;
                            ThisEvent.EventType = ES_NO_EVENT;
                        }
                        break;
                    ...
                    case ES_TIMEOUT:
                        nextState = SEARCH_CASTLE_NW_SEARCH;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                        break;
*/
            ...
            break;
/*
        case SEARCH_CASTLE_NW_DEADBOT_TURNING:
            switch(ThisEvent.EventType)
            {
                case ES_ENTRY:
 ...
 */
        ...
        case SEARCH_CASTLE_NW_RETURN_CENTER:
            switch(ThisEvent.EventType)
            {
                case ES_ENTRY:
    ...
    if (makeTransition)
    {
        SEARCH_CASTLE_NW_RunSubHSM(EXIT_EVENT);
        currentState = nextState;
        SEARCH_CASTLE_NW_RunSubHSM(ENTRY_EVENT);
    }
